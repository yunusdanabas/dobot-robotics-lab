"""
Robot helpers for ME403 Lab 01 (forward kinematics).

Supports Magician or MG400, simulation on by default. For Lab 01,
MG400 goes to firmware joint home (0,0,0,0) after connect so it matches Magician
joint home, not the usual Cartesian ready pose.

Use setup(), move_joints(robot, q), get_pose(robot), get_joints(robot),
move_and_get_feedback(robot, q), and teardown(robot). moveMagician and moveMG400
are aliases for move_joints.

Env: DOBOT_ROBOT_TYPE (magician or mg400), DOBOT_SIMULATION, DOBOT_SIM_BACKEND.
"""
...
# kinematics.py fk_mg400_api). Use MG400_API_* in Lab 1 fk_predict vs get_pose.
# Planar L1/L2/Z_base in ROBOT_MODELS["mg400"] are for teaching only.
MG400_API_L1 = 174.5
MG400_API_L2 = 175.5
MG400_API_C_R = 108.0
MG400_API_C_Z = -52.0
...
def set_tool_offset(bot, ox: float, oy: float, oz: float) -> None:
    """Write TCP offset in mm to Magician firmware (protocol 60). Magician only."""
...
def _mg400_joint_home(move_api) -> None:
    """Firmware joint zero; same idea as Magician joint home for Lab 01."""
    move_api.JointMovJ(0.0, 0.0, 0.0, 0.0)
    move_api.Sync()
...
# ---------------------------------------------------------------------------
# MG400 adapter (Lab 01: joint home after enable)
# ---------------------------------------------------------------------------
...
def _setup_mg400() -> RobotSession:
    if USE_SIMULATION:
        ...
        if hasattr(move_api, "interpolate_joints_to"):
            move_api.interpolate_joints_to(
                [0.0, 0.0, 0.0, 0.0], steps=_SIM_INTERPOLATE_STEPS, pause=_SIM_INTERPOLATE_PAUSE_S
            )
            move_api.Sync()
        else:
            _mg400_joint_home(move_api)
        return RobotSession(
            "mg400",
            simulation=True,
            backend=SIM_BACKEND,
...
def setup() -> RobotSession:
    """Connect or start sim, then move to joint home (0,0,0,0)."""
    configure()
    print(
...
def teardown(robot: RobotSession) -> None:
    """Send joint home, then close sim or hardware."""
    ...
    try:
        if robot.type == "magician":
            ...
            home_body = [0.0, 0.0, 0.0, 0.0]
            if sim_interp and hasattr(mv, "interpolate_joints_to") and _body_max_joint_err_deg(robot, home_body) > _JOINT_SKIP_TOL_DEG:
                mv.interpolate_joints_to(home_body, steps=_SIM_INTERPOLATE_STEPS, pause=_SIM_INTERPOLATE_PAUSE_S)
                if hasattr(mv, "Sync"):
                    mv.Sync()
            else:
                _mg400_joint_home(mv)
    except Exception:
        pass
