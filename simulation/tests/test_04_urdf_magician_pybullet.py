"""Magician URDF pose hook in PyBullet (not cached FK). Run as a script; unittest discover skips these."""
...
def _mode_home_regression(gui: bool) -> tuple[bool, str]:
    ...
    for mode, (ex, ey, ez) in expected.items():
        bot = SimDobot(backend="pybullet", gui=gui, tool=mode)
        try:
...
def main():
    ...
    print(f"=== Magician URDF in PyBullet (GUI={'on' if gui else 'off'}) ===")

    from sim_dobot import SimDobot

    try:
        bot = SimDobot(backend="pybullet", gui=gui)
    except (ImportError, FileNotFoundError, RuntimeError) as exc:
        print(f"  ERROR - PyBullet backend unavailable: {exc}")
        return 1
