...
#include "mg400_plugin/dashboard_api/positive_solution.hpp"

namespace mg400_plugin
{

void PositiveSolution::configure(
  ...
  const mg400_interface::MG400Interface::SharedPtr mg400_if)
{
  ...
  this->srv_ = rclcpp::create_service<ServiceT, CallbackT>(
    ...
    "positive_solution",
    std::bind(&PositiveSolution::onServiceCall, this, _1, _2),
    rclcpp::ServicesQoS().get_rmw_qos_profile(),
    this->node_base_if_->get_default_callback_group());
}

void PositiveSolution::onServiceCall(
  ...
  ServiceT::Response::SharedPtr res)
{
  ...
  if (this->mg400_interface_->ok()) {
    try {
      const auto poses = this->commander_->positiveSolution(
        req->j1, req->j2, req->j3, req->j4,
        req->user, req->tool);
      res->x = poses[0];
      res->y = poses[1];
      res->z = poses[2];
